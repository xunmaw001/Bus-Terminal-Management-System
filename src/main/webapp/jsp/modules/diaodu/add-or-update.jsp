    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑车辆调度</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑车辆调度</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">车辆调度信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="carId" name="carId" type="hidden">
                                   <div class="form-group col-md-6">
                                       <label>排班</label>
                                       <select id="schedulingTypesSelect" name="schedulingTypes" class="form-control">
                                       </select>
                                   </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("diaodu/" + urlParam, "POST", data, (res) => {
                if(res.code == 0)
                {
                    window.sessionStorage.removeItem('adddiaodu');
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationSchedulingtypesSelect(){
            var schedulingTypesSelect = document.getElementById('schedulingTypesSelect');
            if(schedulingTypesSelect != null && schedulingTypesOptions != null  && schedulingTypesOptions.length > 0 ){
                for (var i = 0; i < schedulingTypesOptions.length; i++) {
                    schedulingTypesSelect.add(new Option(schedulingTypesOptions[i].indexName,schedulingTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(schedulingTypesSelect != null && schedulingTypesOptions != null  && schedulingTypesOptions.length > 0 ) {
            for (var i = 0; i < schedulingTypesOptions.length; i++) {
                if (schedulingTypesOptions[i].codeIndex == ruleForm.schedulingTypes) {//下拉框value对比,如果一致就赋值汉字
                        schedulingTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                carId: "required",
                schedulingTypes: "required",
            },
            messages: {
                carId: "车辆id不能为空",
                schedulingTypes: "排班不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var adddiaodu = window.sessionStorage.getItem("adddiaodu");
        if (adddiaodu != null && adddiaodu != "" && adddiaodu != "null") {
            window.sessionStorage.removeItem('adddiaodu');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("diaodu/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationSchedulingtypesSelect();
            <!--  初始化级联表的下拉框  -->
