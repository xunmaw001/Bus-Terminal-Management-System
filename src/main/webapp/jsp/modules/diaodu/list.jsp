                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <div class="table-responsive mb-3">
                            <div class="col-sm-12">
                                            ...
                                            <label>
                                                排班
                                                <select name="schedulingTypesSelectSearch" id="schedulingTypesSelectSearch" class="form-control form-control-sm"
                                        ...
                                        <label>
                                            车辆名字
                                            <input type="text" id="carNameSearch" class="form-control form-control-sm"
                                                   placeholder="" aria-controls="tableId">
                                        </label>
                                             
                                        <label>
                                            司机
                                            <input type="text" id="carDriverSearch" class="form-control form-control-sm"
                                                   placeholder="" aria-controls="tableId">
                                        </label>
                                        ...
                                        <label>
                                            车牌号
                                            <input type="text" id="carNumberSearch" class="form-control form-control-sm"
                                                   placeholder="" aria-controls="tableId">
                                        </label>
...
<script>
    ...
    function search() {
        ...
        if( endSearchInput != null){
            ...
            //排班
        var schedulingTypesSelectSearchInput = document.getElementById("schedulingTypesSelectSearch");
        if(schedulingTypesSelectSearchInput != null){
            var schedulingTypesIndex = schedulingTypesSelectSearchInput.selectedIndex;
            var schedulingTypesValue;
            if( schedulingTypesIndex  == 0){
                schedulingTypesValue='';
            }else{
                schedulingTypesValue= document.getElementById("schedulingTypesSelectSearch").options[schedulingTypesIndex].value;
            }
            searchForm.schedulingTypes = schedulingTypesValue;
        }
            getDataList();
    }
    ...
    function getDataList() {
        http("diaodu/page", "GET", {
            ...
            order: sortOrder,
            //本表的
            schedulingTypes: searchForm.schedulingTypes,
    ...
    function setDataRow(item, number) {
        ...
        row.appendChild(carNameCell);
                ...
                //司机
        var carDriverCell = document.createElement('td');
        carDriverCell.innerHTML = item.carDriver;
        row.appendChild(carDriverCell);


                //车辆描述
        var repairContentCell = document.createElement('td');
        //如果图文详情包含图片
        if (item.repairContent && item.repairContent.indexOf('img') != -1) {
            //暂时只考虑图片+文字 和 文字+图片的情况
            var beginIndex = item.repairContent.indexOf('<img');
            var endIndex = item.repairContent.indexOf('>');
                repairContentCell.innerHTML = item.repairContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
        } else {
            if (item.repairContent != null && item.repairContent != "" && item.repairContent.length >= 11) {
                    repairContentCell.innerHTML = item.repairContent.substring(0, 10) + "...";
            } else {
                    repairContentCell.innerHTML = item.repairContent;
            }
        }
        row.appendChild(repairContentCell);
        ...
        row.appendChild(endCell);
                ...
                //车票金额
        var sellMoneyCell = document.createElement('td');
        sellMoneyCell.innerHTML = item.sellMoney;
        row.appendChild(sellMoneyCell);



        //车辆id
        var carIdCell = document.createElement('td');
        carIdCell.innerHTML = item.carId;
        row.appendChild(carIdCell);


        //排班
        var schedulingTypesCell = document.createElement('td');
        schedulingTypesCell.innerHTML = item.schedulingValue;
        row.appendChild(schedulingTypesCell);
    ...
    //填充本表搜索下拉框
        ...
        function schedulingTypesSelectSearch() {
            var schedulingTypesSelectSearch = document.getElementById('schedulingTypesSelectSearch');
            if(schedulingTypesSelectSearch != null) {
                schedulingTypesSelectSearch.add(new Option('',''));
                if (schedulingTypesOptions != null && schedulingTypesOptions.length > 0){
                    for (var i = 0; i < schedulingTypesOptions.length; i++) {
                            schedulingTypesSelectSearch.add(new Option(schedulingTypesOptions[i].indexName,schedulingTypesOptions[i].codeIndex));
                    }
                }
            }
        }
    ...
    //查询当前表搜索条件所有列表
        function schedulingTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=scheduling_types", "GET", {}, (res) => {
                if(res.code == 0){
                    schedulingTypesOptions = res.data.list;
                }
            });
        }
