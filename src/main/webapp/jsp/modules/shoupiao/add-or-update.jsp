            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑售票</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">售票信息</h3>
                        <form id="addOrUpdateForm">
                            <div class="form-row">
                            <!-- 级联表的字段 -->
                                    <div id="bbbb" class="form-group col-md-6">
                                        <label>车辆</label>
                                        <div>
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="start" name="start" class="form-control"
                                               placeholder="始发站" readonly>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>终点站</label>
                                        <input id="end" name="end" class="form-control"
                                               placeholder="终点站" readonly>
                                    </div>
                            <!-- 当前表的字段 -->
                                ...
                                <input id="carId" name="carId" type="hidden">
                                 <div class="form-group col-md-6">
                                     <label>售卖金额</label>
                                     <input id="actualMoney" name="actualMoney" class="form-control"
                                            onchange="actualMoneyChickValue(this)" placeholder="售卖金额">
                                 </div>
                                <div id="aaaa" class="form-group col-md-6">
                                    <label>售票状态</label>
                                    <select id="shoupiaoTypesSelect" name="shoupiaoTypes" class="form-control">
                                    </select>
                                </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("shoupiao/" + urlParam, "POST", data, (res) => {
                if(res.code == 0)
                {
                    window.sessionStorage.removeItem('addshoupiao');
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationShoupiaotypesSelect(){
            var shoupiaoTypesSelect = document.getElementById('shoupiaoTypesSelect');
            if(shoupiaoTypesSelect != null && shoupiaoTypesOptions != null  && shoupiaoTypesOptions.length > 0 ){
                for (var i = 0; i < shoupiaoTypesOptions.length; i++) {
                    shoupiaoTypesSelect.add(new Option(shoupiaoTypesOptions[i].indexName,shoupiaoTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(shoupiaoTypesSelect != null && shoupiaoTypesOptions != null  && shoupiaoTypesOptions.length > 0 ) {
            for (var i = 0; i < shoupiaoTypesOptions.length; i++) {
                if (shoupiaoTypesOptions[i].codeIndex == ruleForm.shoupiaoTypes) {//下拉框value对比,如果一致就赋值汉字
                        shoupiaoTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function actualMoneyChickValue(e){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                shoupiaoTypes: "required",
            },
            messages: {
                carId: "车辆id不能为空",
                actualMoney: "售卖金额不能为空",
                shoupiaoTypes: "售票状态不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addshoupiao = window.sessionStorage.getItem("addshoupiao");
        if (addshoupiao != null && addshoupiao != "" && addshoupiao != "null") {
            window.sessionStorage.removeItem('addshoupiao');
            //注册表单验证
            $(validform());
            ...
            if(role =="检票管理员" ){
                ...
                $('#aaaa').remove();
                $('#bbbb').remove();
                $('#actualMoney').attr('style', 'pointer-events: none;');
            }else if(role =="管理员" ){
                $('#submitBtn').text('修改');
            }
            ...
            window.sessionStorage.removeItem('updateId');
            http("shoupiao/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#actualMoney").val(ruleForm.actualMoney);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationShoupiaotypesSelect();
            <!--  初始化级联表的下拉框  -->
